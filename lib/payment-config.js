...
function resolveAllowedOrigin() {
  ...
  if (process.env.VERCEL_ENV && value.startsWith('https://')) {
    let previewOrigin;
    try {
      previewOrigin = new URL(value);
    } catch (error) {
      throw new PaymentConfigError('PAYMENT_ALLOWED_ORIGIN must be an exact HTTPS Vercel preview origin.');
    }
    if (!previewOrigin.hostname.endsWith('.vercel.app')
      || previewOrigin.origin !== value
